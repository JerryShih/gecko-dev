...
bool
CompositorParent::VsyncTick(TimeStamp aTimestamp,
                            uint64_t aFrameNumber)
{
  // Post Tick task to comositor thread
  CompositorLoop()->PostTask(FROM_HERE,
                             NewRunnableMethod(this,
                             &CompositorParent::VsyncTickInternal,
                             aTimestamp,
                             aFrameNumber));
...
void
CompositorParent::VsyncTickInternal(TimeStamp aTimestamp,
                                    uint64_t aFrameNumber)
{
...
CompositorParent::ActorDestroy(ActorDestroyReason why)
{
  if (gfxPrefs::FrameUniformityEnabled() &&
      gfxPrefs::FrameUniformityCompositorVsyncEnabled())
  {
    // sync unregister
    VsyncDispatcher::GetInstance()->GetCompositorRegistry()->RemoveObserver(this, false);
  }
  CancelCurrentCompositeTask();
...
CompositorParent::ScheduleComposition()
{
      ...
      gfxPrefs::FrameUniformityCompositorVsyncEnabled())
  {
    ...
    // Add compositer to vsync dispatcher
    VsyncDispatcher::GetInstance()->GetCompositorRegistry()->AddObserver(this, false);
