...
void
D3D11TextureData::SyncWithObject(TextureSyncObject* aSyncObject)
{
  ...
  MOZ_ASSERT(aSyncObject->GetSyncType() == TextureSyncObject::SyncType::D3D11);
  SyncObjectD3D11Texture* sync = static_cast<SyncObjectD3D11Texture*>(aSyncObject);
  ...
  switch (mFormat) {
    ...
    case gfx::SurfaceFormat::B8G8R8X8: {
      ...
      aAPI->AddExternalImage(aImageKeys[0],
                             descriptor,
                             aExtID,
                             WrExternalImageBufferType::Texture2DHandle,
                             0);
      break;
    }
    case gfx::SurfaceFormat::NV12: {
...
SyncObjectD3D11Renderer::SyncObjectD3D11Renderer(ID3D11Device* aDevice)
 : mSyncHandle(0)
 , mDevice(aDevice)
{
  MOZ_ASSERT(aDevice);
...
SyncHandle
SyncObjectD3D11Renderer::GetSyncHandle()
{
  return mSyncHandle;
}

bool
SyncObjectD3D11Renderer::Synchronize()
{
  HRESULT hr;
...
SyncObjectD3D11Texture::SyncObjectD3D11Texture(SyncHandle aSyncHandle, ID3D11Device* aDevice)
...
void
SyncObjectD3D11Texture::RegisterTexture(ID3D11Texture2D* aTexture)
{
  mSyncedTextures.push_back(aTexture);
}

bool
SyncObjectD3D11Texture::IsSyncObjectValid()
{
...
void
SyncObjectD3D11Texture::Synchronize()
{
