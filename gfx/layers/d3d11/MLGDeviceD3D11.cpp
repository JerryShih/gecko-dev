...
MLGDeviceD3D11::GetTextureFactoryIdentifier() const
{
  ...
  if (mRendererSyncObject) {
    ident.mSyncHandle = mRendererSyncObject->GetSyncHandle();
  }
...
{
  MOZ_ASSERT(!mRendererSyncObject);
  MOZ_ASSERT(mDevice);

  mRendererSyncObject = RendererSyncObject::CreateRendererSyncObject(mDevice);
  MOZ_ASSERT(mRendererSyncObject);

  return mRendererSyncObject->Init();
}
...
MLGDeviceD3D11::Synchronize()
{
  ...
  if (mRendererSyncObject) {
    if (!mRendererSyncObject->Synchronize()) {
