                                      ...
                                      uint64_t aFrameNumber)
{
  ...
  ObserverList tempObserverList1(mTemporaryObserverList);
  ObserverList tempObserverList2(mObserverList);

  // The TickVsync() is not always triggered. We will not tick the refresh driver
  // unless it registers again.
  mTemporaryObserverList.Clear();
  ...
  for (ObserverList::size_type i = 0; i < tempObserverList1.Length(); i++) {
    tempObserverList1[i]->TickVsync(aTimestampNanosecond, aTimestamp, aTimestampJS, aFrameNumber);
  }
  for (ObserverList::size_type i = 0; i < tempObserverList2.Length(); i++) {
    tempObserverList2[i]->TickVsync(aTimestampNanosecond, aTimestamp, aTimestampJS, aFrameNumber);
  }
