...
#ifndef mozilla_widget_shared_VsyncDispatcherTrace_h
#define mozilla_widget_shared_VsyncDispatcherTrace_h
...
private:
  ...
  public:
    ...
    LogData(DataType aDataValue, uint32_t aFrameNumber, bool aCorner)
    {
  ...
  void PrintRawDefault(const char* aMsg, uint32_t aFrameNumber, DataType aDataValue, bool aCorner = false)
  {
    if (aCorner){
      return;
    }
    printf_stderr("%-20s, %u, %g%s",
                  aMsg,
                  aFrameNumber,
                  aDataValue, aCorner?", corner":"");
  }
  ...
  {
    printf_stderr("%-20s, avg:%5.3f, std:%5.3f, cv:%5.3f%% max:(%d, %4.2f), min:(%d, %4.2f)",
                  aMsg,
                  aAvg,
                  aStd,
                  aStd / aAvg * 100.0f,
...
#endif  //mozilla_widget_shared_VsyncDispatcherTrace_h
