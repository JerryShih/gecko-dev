...
GeckoTouchDispatcher::~GeckoTouchDispatcher()
{
  VsyncDispatcher::GetInstance()->GetInputDispatcherRegistry()
                                ->RemoveObserver(this, true);
}
...
bool
GeckoTouchDispatcher::VsyncTick(TimeStamp aTimestamp,
                                uint64_t aFrameNumber)
{
  ...
  if (haveTouchData) {
    TimeDuration diff = aTimestamp - TimeStamp();
    NS_DispatchToMainThread(new DispatchTouchEventsMainThread(this, diff.ToMicroseconds() * 1000));
  }
...
GeckoTouchDispatcher::DispatchTouchEvent(MultiTouchInput& aMultiTouch)
{
  if ((aMultiTouch.mType == MultiTouchInput::MULTITOUCH_END ||
    ...
    MutexAutoLock lock(mTouchQueueLock);
    mTouchMoveEvents.clear();
    VsyncDispatcher::GetInstance()->GetInputDispatcherRegistry()->RemoveObserver(this, true);
